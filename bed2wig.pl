...
my $usage = "Usage: $0 --bedgraph <infile.bedgraph> --genomeSize <genomeSize.file> --wig <outfile.wig> --step <step_size> [--compact]\n";

# Parse command line arguments

my $infile; # bedGraph input file name
my $genome; # genomeSize input file name
...
print("Start to read genome size file\n");
# Open Genome Size file
my %size = ();
open(GENOME, '<',$genome) || die "Could not open $genome: $!\n";
while(<GENOME>){
  chomp;
  my ($chr,$size) = split(/\t/);
  $size{$chr} = $size;
...
while (<IN>) {
  ...
  if ($chr ne $cur_chr) {
    if(exists $size{$cur_chr}){
      my $end_pos = $size{$cur_chr};
      #print("$next_pos\t$end_pos\n");
      # Print values when last step is smaller than the chromosome size
      while ($next_pos < $end_pos + $step ){
        $cur_val = 0;
  ...
  while ($end >= $next_pos) {
    $cur_val += $val * ($next_pos - max($cur_pos, $start))/($end - $start);
    print_wig_line($cur_chr, \$cur_pos, \$next_pos, \$exp_pos, \$cur_val, $chr, $start, $end, $val, $step);
  }
  ...
  if ($end < $next_pos) {
    ...
    $cur_val += $val * ($end - max($cur_pos, $start))/($end - $start);
  }
